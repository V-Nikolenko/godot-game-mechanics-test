...
**Correction (2026-09-16, plan review finding B7).** The original wording of that note — "nothing
below is attached to a page that was not actually read" — was **not true of finding 9**. The two
Zelda Dungeon pages it cites are Cloudflare-gated from this container: WebFetch returns 403 and
`scripts/fetch-page.sh` returns a CAPTCHA interstitial. Their figures were confirmed instead from a
Wayback capture and cross-checked against `zelda.fandom.com` and `zeldawiki.wiki` — the **numbers
were and are correct**, but the row presented reconstructed prose inside quotation marks, which is
the one error that makes every other quote in a research document unverifiable by default. Finding 9
is now a marked paraphrase with its retrieval method stated. Every other quoted passage in this
document was independently spot-checked verbatim during the review (findings 1, 3, 4, 7, 8) and
holds.
