...
hub permanently raises the ship's charge capacity, and that capacity persists across runs exactly
like the permanent shield count already does. The blue afterburner flame the ship already owns
...
2. **Holding W sustains a boost; releasing it bleeds.** At 700 px/s the existing damping removes
   ~420 px/s² while W adds 380 px/s², so a coasting boost decays noticeably inside the hold window
   and a thrusting one nearly holds. That is free, readable depth, and it keeps W meaningful.
3. **A module boost still wins.** `_handle_thrust()` returns early while `engine_boost_active`, so
   the core boost is simply not reachable during a module dash — no double-write, and (see below)
   no charge spent either.
...
|---|---|---|
| `boost_exit_speed` | **700.0** px/s | **Revised** from research's 600. Must exceed `max_speed = 420` (finding 1's tradeoff). 600 is only 1.43× cruise, and with the ceiling decaying over 0.6 s that buys ~54 px of extra travel over simply cruising — not a "significant burst". 700 with the hold window below buys ~196 px. Still 0.47× `EngineBoostModule`'s 1500 and 0.42× its 467 px of travel, so the module stays the heavier ability. |
...
Everything below asserts **intent**, not characterization — this is new code. Tests that touch
`ShipProgressionState` must use `tests/helpers/save_sandbox.gd` (`tests/README.md`). Tree-less
component instances are `free()`d in `after_each`, and `scripts/check-test-leaks.sh` is run after
any step that awaits.

### `tests/integration/test_open_space_boost_verb.gd` — step 1

Instantiates `player_ship.tscn`, `set_physics_process(false)`, drives `_step_boost()` by hand.
Note that `Input.is_action_pressed()` returns `false` headless, so `_handle_thrust(delta)` is
callable in a test and behaves as "no keys held".
...
| A module boost wins | `engine_boost_active = true`; `_step_boost(true, d)` leaves `velocity` unchanged. |
...
| **Step 4 — the mid-session upgrade** | `bind_progression = true`, sandboxed autoload: `max_charges` starts at `MIN_BOOST_CHARGES`; emitting `boost_charge_count_changed(3)` raises `max_charges` to 3 **and** grants the extra charge immediately. |
...
| **Boundary: a boost refused by a module burns no charge** | `engine_boost_active = true`, full meter; `_step_boost(true, d)` → `charges` unchanged. |
| The ship drives the meter | The meter does not tick itself: after a spend, repeated `ship._handle_thrust(d)` calls restore the charge over `recharge_delay_sec + 1/recharge_rate`. |
...
| Segments follow capacity | Emitting `charges_changed(2.0, 4)` leaves the bar rendering 4 segments. |
...
| **It actually does something** | A real instance is collected against the live (sandboxed) autoload and `boost_charge_count` goes up by 1. Every placement test passes on a pickup whose `_collect()` is empty; this is the case that does not. |
| **Boundary: collecting at the cap** | At `MAX_BOOST_CHARGES`, collecting leaves the count at the cap and logs no error. |
