...
func present_line(line: DialogLineResource) -> void:
	_trace("present_line enter  state=%d  active_bar=%s  top_bar=%s  bot_bar=%s" % [
	...
	_trace("starting fade-in tween  active_bar=%s  tree_paused=%s" % [
	...
	# Reveal the text.
	_state = State.TYPING
	_trace("starting text reveal  reveal_mode=%d  text_len=%d" % [line.reveal, line.text.length()])
	match line.reveal:
		DialogLineResource.Reveal.TYPEWRITER:
			var duration: float = max(line.text.length() / max(line.typing_speed, 1.0), 0.05)
			_trace("typewriter tween  duration=%.2f" % duration)
			...
			_typing_tween.finished.connect(func() -> void:
				_trace("typewriter tween finished naturally")
				if _state == State.TYPING:
					_state = State.READY
					typing_completed.emit()
			, CONNECT_ONE_SHOT)
			await typing_completed
			_trace("typing_completed received  state=%d" % _state)
		...
		DialogLineResource.Reveal.FADE_IN:
			...
			await t.finished
			_trace("fade-in text tween DONE")
		DialogLineResource.Reveal.INSTANT:
			_active_text.visible_ratio = 1.0
			_trace("instant reveal DONE")

	## Reached only by FADE_IN and INSTANT paths.
	_state = State.READY
	_trace("state=READY, emitting typing_completed")
	typing_completed.emit()
...
func skip_typing() -> void:
	if _state != State.TYPING:
		return
	_trace("skip_typing — killing tween, emitting typing_completed")
	if _typing_tween:
...
## Per-line/per-tween tracing. Off unless Godot was started with `--verbose` — several of
## these fire per dialog line, including from inside tween callbacks.
